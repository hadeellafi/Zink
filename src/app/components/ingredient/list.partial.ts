...
export class IngredientListPartial implements OnInit {
    ...
    constructor(private ingredientStateService: IngredientState, private ingredientService: IngredientService, private toast: ToastState) { }
    ngOnInit(): void {
        this.ingredients$ = this.ingredientService.GetIngredients(this.bitId).pipe(
            switchMap(ingredients => this.ingredientStateService.SetList(ingredients)));
    }
