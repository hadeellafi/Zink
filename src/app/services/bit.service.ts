...
export class BitService {
  ...
  GetBits(): Observable<IBit[]> {
    return this.http
      .get(this._listUrl)
      .pipe(map((response: any) => Bit.NewInstances(response)));
  }
  ...
  GetBit(id: string): Observable<IBit> {
    return this.http
      ...
      .pipe(map((response: any) => Bit.NewInstance(response)));
  }
  ...
  DeleteBit(bit: IBit): Observable<boolean> {
    return this.http
      .delete(this._deleteUrl.replace(':id', bit.id))
      .pipe(map((response: any) => response));
  }
  CreateBit(bit: IBit): Observable<IBit> {
    return this.http
      .post(this._createUrl, bit)
      .pipe(map((response: any) => Bit.NewInstance(response)));
  }
  UpdateBit(bit: IBit): Observable<IBit> {
    return this.http
      .put(this._updateUrl.replace(':id', bit.id), bit)
      .pipe(map((response: any) => Bit.NewInstance(response)));
  }
