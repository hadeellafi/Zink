...
export class IngredientService {
  ...
  GetIngredients(bitId: string): Observable<IIngredient[]> {
    return this.http
      .get(this._listUrl.replace(':id', bitId))
      .pipe(map((response: any) => Ingredient.NewInstances(response)));
  }

  GetIngredient(id: string): Observable<IIngredient> {
    return this.http
      .get(this._detailUrl.replace(':id', id))
      .pipe(map((response: any) => Ingredient.NewInstance(response)));
  }

  CreateIngredient(ingredient: IIngredient): Observable<IIngredient> {
    return this.http
      .post(this._createUrl, ingredient)
      .pipe(map((response: any) => Ingredient.NewInstance(response)));
  }

  UpdateIngredient(ingredient: IIngredient): Observable<IIngredient> {
    return this.http
      .put(this._updateUrl.replace(':id', ingredient.id), ingredient)
      .pipe(map((response: any) => Ingredient.NewInstance(response)));
  }
  ...
  DeleteIngredient(
    ...
    const url = this._deleteUrl
      .replace(':id', bitId)
      .replace(':iid', ingredient.id);
