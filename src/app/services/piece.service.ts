...
export class PieceService {
  ...
  GetPieces(): Observable<IPiece[]> {
    return this.http
      .get(this._listUrl)
      .pipe(map((response: any) => Piece.NewInstances(response)));
  }

  GetPiece(id: string): Observable<IPiece> {
    return this.http
      .get(this._detailUrl.replace(':id', id))
      .pipe(map((response: any) => Piece.NewInstance(response)));
  }
  ...
  CreatePiece(piece: IPiece): Observable<IPiece> {
    return this.http
      .post(this._createUrl, piece)
      .pipe(map((response: any) => Piece.NewInstance(response)));
  }
  UpdatePiece(piece: IPiece): Observable<IPiece> {
    return this.http
      .put(this._updateUrl.replace(':id', piece.id), piece)
      .pipe(map((response: any) => Piece.NewInstance(response)));
  }
  DeletePiece(id: string): Observable<boolean> {
    return this.http
      .delete(this._deleteUrl.replace(':id', id))
